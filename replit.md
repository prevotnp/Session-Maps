...
- **Routing**: Offers Direct, Road, Trails (OpenRouteService foot-hiking with fallback to custom Dijkstra), and Draw modes. Routes can be manually shaped with control points. Real-time distance and stats recalculation. GPS activity recording generates routes. Trail overlay uses Waymarked Trails raster tiles. AI route generation queries real trail data from Waymarked Trails API + Overpass before calling Claude.
...
- **OpenRouteService**: Professional foot-hiking routing (optional, requires ORS_API_KEY).
- **Waymarked Trails**: Color-coded hiking trail overlay tiles and named route API.
